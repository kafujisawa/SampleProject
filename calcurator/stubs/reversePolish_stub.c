...
/** User stub definition for function: int reversePolish(char *, char *) */
EXTERN_C_LINKAGE int reversePolish (char * ptr, char  outstr[256]) ;
EXTERN_C_LINKAGE int CppTest_Stub_reversePolish (char * ptr, char  outstr[256]) 
{
#if CPPTEST_TRIGGER_ENABLED
    int __return = 0;
    int __callOrig = 0;

    CPPTEST_ACTUAL_CALL_C("reversePolish")->WithRefArg("ptr", &ptr, tgr_ptr_type(tgr_char_type()))->WithRefArg("outstr", outstr, tgr_ptr_type(tgr_char_type()))->WithRefArg("__return", &__return, tgr_int_type())->WithRefArg("__callOrig", &__callOrig, tgr_int_type())->End();
    if (__callOrig) {
        int __return = reversePolish(ptr, outstr);
        CPPTEST_ACTUAL_AFTER_CALL_C("reversePolish")->WithRefArg("ptr", &ptr, tgr_ptr_type(tgr_char_type()))->WithRefArg("outstr", outstr, tgr_ptr_type(tgr_char_type()))->WithRefArg("__return", &__return, tgr_int_type())->End();
        return __return;
    }
    return __return;
#else
