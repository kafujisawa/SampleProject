...
/** User stub definition for function: int formatCheck(char *) */
EXTERN_C_LINKAGE int formatCheck (char * instr) ;
EXTERN_C_LINKAGE int CppTest_Stub_formatCheck (char * instr) 
{
#if CPPTEST_TRIGGER_ENABLED
    int __return = 0;
    int __callOrig = 0;

    CPPTEST_ACTUAL_CALL_C("formatCheck")->WithRefArg("instr", &instr, tgr_ptr_type(tgr_char_type()))->WithRefArg("__return", &__return, tgr_int_type())->WithRefArg("__callOrig", &__callOrig, tgr_int_type())->End();
    if (__callOrig) {
        int __return = formatCheck(instr);
        CPPTEST_ACTUAL_AFTER_CALL_C("formatCheck")->WithRefArg("instr", &instr, tgr_ptr_type(tgr_char_type()))->WithRefArg("__return", &__return, tgr_int_type())->End();
        return __return;
    }
    return __return;
#else
